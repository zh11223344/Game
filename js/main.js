...
// const sidebar = document.querySelector(".poko-sidebar");
// const overlay = document.querySelector(".poko-sidebar .overlay");
// if (menu !== null) {
...
// var sideBarLists = document.querySelectorAll(".poko-sidebar-body li");
...
//   if (toggler.checked) {
//     document.documentElement.setAttribute("poko-theme", "dark");
//     localStorage.setItem("zon_theme", "dark");
//     darkIcon.classList.add("active-icon");
//     lightIcon.classList.remove("active-icon");
//   } else {
//     document.documentElement.setAttribute("poko-theme", "light");
...
function LikeGame(game_id) {
  ...
  xhr.open("POST", `${window.poko.url}xhr/like.php`);
...
if (lForm !== null) {
  lForm.addEventListener("submit", (e) => {
    ...
    xhr.open("POST", `${window.poko.url}xhr/login.php`);
    xhr.onreadystatechange = () => {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
...
if (RForm !== null) {
  RForm.addEventListener("submit", (e) => {
    ...
    const xhr = new XMLHttpRequest();
    button.setAttribute("disabled", "true");
    xhr.open("POST", `${window.poko.url}xhr/register.php`);
    xhr.onreadystatechange = () => {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          if (xhr.responseText.includes("successfully") == true) {
...
if (upForm !== null) {
  upForm.addEventListener("submit", (e) => {
    ...
    const xhr = new XMLHttpRequest();
    button.setAttribute("disabled", "true");
    xhr.open("POST", `${window.poko.url}xhr/update-userinfo.php`);
    xhr.onreadystatechange = () => {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          console.log(xhr.responseText);
